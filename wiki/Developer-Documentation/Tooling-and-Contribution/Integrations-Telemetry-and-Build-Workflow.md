...
- Tamework uses the conventional project descriptor loaded by `CrashTelemetryService`. The embedded Patchwork `1.3.3` runtime contributes a separate logical `patchwork` project through the generic contribution API. Patchwork's contribution is hosted-only in this release.
...
- Gradle packages Tamework's direct Telemetry edge and Patchwork's transitive edge at the aligned `1.2.1` runtime. `patchwork_version=1.3.3` and `alecstelemetry_version=1.2.1` are the source-of-truth properties.
