...
Formation positions turn with a smoothed leader travel heading. Followers match the leader's measured velocity and apply bounded position correction. Membership order determines follower positions; joins, departures, or a replacement leader may rearrange them. Loose groups spread in two horizontal dimensions and add small, slow drift. Existing NPC separation remains active.

The body motion `TameworkFlightFormation` accepts `Formation`, `Spacing`, `Tightness`, and `RelativeSpeed`. `RelativeSpeed` defaults to `0.8` and caps the additional correction as a fraction of the follower's speed limit at its current pitch; total requested speed is capped at that limit. It is not a speed multiplier for the leader. A follower cannot catch a leader that is already moving at the follower's maximum speed.
