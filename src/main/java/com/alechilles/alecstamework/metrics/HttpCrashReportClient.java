...
public final class HttpCrashReportClient implements CrashReportClient {
    ...
    public HttpCrashReportClient(@Nonnull String endpoint,
    ...
    @Nonnull
    @Override
    public UploadResult upload(@Nonnull String payloadJson) {
        if (endpoint.isBlank()) {
            return UploadResult.failure(0, "Endpoint is not configured.");
        }
        HttpURLConnection http = null;
        try {
            http = (HttpURLConnection) URI.create(endpoint).toURL().openConnection();
            ...
            if (logger != null) {
                logger.at(Level.WARNING).withCause(ex).log(
                        "Crash telemetry upload request failed."
                );
            }
