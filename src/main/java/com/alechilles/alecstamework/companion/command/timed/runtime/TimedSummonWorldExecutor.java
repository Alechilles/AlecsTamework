...
final class TimedSummonWorldExecutor {
    ...
    private CompletionStage<LiveOperationResult> store(
        ...
        long receiptChunk = probe.receipt().chunkIndex();
        return probe.source().status() == EvidenceStatus.ABSENT
                ? persistRetirement(
    ...
    private CompletionStage<LiveOperationResult> afterReceiptPersistence(
        ...
        return probe.source().status() == EvidenceStatus.ABSENT
                ? persistRetirement(
                        authority, attempts, expectedChunk
                )
                : retire(authority, attempts, expectedChunk);
    }

    private CompletionStage<LiveOperationResult> retire(
            TimedSummonWorldAuthority.Store authority,
            AttemptGateway attempts,
            long receiptChunk
    ) {
        ...
        if (mutation.status() == MutationStatus.EXACT) {
            return Objects.equals(mutation.chunkIndex(), receiptChunk)
                    ...
                    : completed(unknown(
                            "store_retirement_chunk_conflict",
                            mutation.cause()
                    ));
        }
        StoreProbe probe = safety.probeStore(attempts, authority);
        LiveOperationResult failure =
                exactReceiptFailure(probe, receiptChunk);
        if (failure != null) {
            return completed(mutation.status() == MutationStatus.CONFLICT
                    ? unknown(
                            "store_retirement_ambiguous",
                            first(mutation.cause(), failure.cause())
                    )
                    : failure);
        }
        ...
        return completed(mutation.status() == MutationStatus.RETRYABLE
                ? retryable(
                        "store_retirement_retryable", mutation.cause()
                )
                : unknown(
                        "store_retirement_ambiguous", mutation.cause()
                ));
    }

    private CompletionStage<LiveOperationResult> persistRetirement(
            TimedSummonWorldAuthority.Store authority,
            AttemptGateway attempts,
            @Nullable Long chunkIndex
    ) {
        if (chunkIndex == null) {
            return completed(unknown(
                    "store_retirement_chunk_missing", null
            ));
        }
        return persistThenResume(
                attempts,
    ...
    private CompletionStage<LiveOperationResult> verifyRetirement(
            TimedSummonWorldAuthority.Store authority,
            AttemptGateway attempts,
            long expectedChunk
    ) {
        ...
        return completed(probe.source().status() == EvidenceStatus.ABSENT
                ? confirmed("store_receipt_and_retirement_saved_exact")
                : retryable(
                        "store_source_still_present_after_save",
                        probe.source().cause()
                ));
    }
