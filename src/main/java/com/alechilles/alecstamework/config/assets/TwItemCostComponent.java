...
/**
 * One item-agnostic component of an ordered AND cost.
 *
 * <p>This value is deliberately independent of command revival so breeding, husbandry, and other
 * systems can share the same authoring and payment contracts.
 */
public final class TwItemCostComponent {
    ...
    public static final TwItemCostComponent[] EMPTY_ARRAY = new TwItemCostComponent[0];
    ...
    public void validate() {
        if (itemId == null || itemId.isBlank()) {
            throw new IllegalArgumentException("Cost ItemId must be non-blank.");
        }
        if (quantity <= 0) {
            throw new IllegalArgumentException("Cost Quantity must be positive for " + itemId + ".");
        }
    }
    ...
    @Nonnull
    public static TwItemCostComponent[] validateAndCopy(@Nullable TwItemCostComponent[] components) {
        if (components == null || components.length == 0) {
            return EMPTY_ARRAY;
        }
        TwItemCostComponent[] copy = new TwItemCostComponent[components.length];
        Set<String> ids = new HashSet<>();
        for (int index = 0; index < components.length; index++) {
            TwItemCostComponent component = components[index];
            if (component == null) {
                throw new IllegalArgumentException("Cost component at index " + index + " is null.");
            }
            component.validate();
            String itemId = component.getItemId();
            if (!ids.add(itemId)) {
                throw new IllegalArgumentException("Duplicate cost ItemId is not allowed: " + itemId);
            }
