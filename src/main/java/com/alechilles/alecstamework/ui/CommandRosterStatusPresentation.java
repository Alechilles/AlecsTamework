...
/** Immutable per-row timed-roster status derived from replacement APIs. */
public record CommandRosterStatusPresentation(
    ...
    public CommandRosterStatusPresentation {
        ...
        if (revision < 0L
                || remainingMs != null && remainingMs < 0L
                || configuredDurationMs < 0L
                || cooldownRemainingMs < 0L
                || activeCount < 0
                || activeLimit < 0) {
            throw new IllegalArgumentException(
                    "Roster presentation values cannot be negative."
            );
        }
    ...
    public boolean capUnlimited() {
        return activeLimit == 0;
    }

    public boolean capBlocked() {
        return activeLimit > 0 && activeCount >= activeLimit;
    }

    public boolean summonVisible() {
        return state == CommandTimedSummoningState.ROSTER_STORED;
    }

    public boolean summonEnabled() {
        return summonVisible()
                && cooldownRemainingMs == 0L
                && !capBlocked();
    }
    ...
    public boolean dismissEnabled() {
        return dismissVisible();
    }

    public boolean paidRevivalState() {
        return state == CommandTimedSummoningState.DEAD_REVIVABLE
                || state == CommandTimedSummoningState.LOST;
    }
