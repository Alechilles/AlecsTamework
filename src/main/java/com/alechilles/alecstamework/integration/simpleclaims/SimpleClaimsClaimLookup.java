...
/**
 * Minimal SimpleClaims 1.0.38 claim-lookup capability used by population policies.
 *
...
final class SimpleClaimsClaimLookup {
    ...
    @Nonnull
    static SimpleClaimsClaimLookup forTypes(@Nonnull Class<?> managerType, @Nonnull Class<?> chunkType) {
        try {
    ...
    private Result invokeLookup(@Nonnull String worldName, int blockX, int blockZ) {
        try {
            ...
            if (chunk == null) {
                return new Result(Status.NO_CLAIM, null, null);
            }
            ...
            return new Result(Status.CLAIM_FOUND, partyId, null);
        } catch (Throwable throwable) {
            return new Result(
                    Status.ERROR,
                    null,
    ...
    private static SimpleClaimsClaimLookup unavailable(@Nonnull String reason) {
        return new SimpleClaimsClaimLookup(false, reason, null, null, null, null);
    }
}
