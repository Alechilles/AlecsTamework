...
final class SpawnerItemDisplayMetadataService {
    ...
    private DisplayPayload buildPayload(ItemStack stack, ItemFeatureConfig config, BsonDocument metadataDoc) {
        ...
        List<ResolvedAttachmentDisplay> attachments = resolveAttachmentDisplays(metadataDoc, roleId);
        Message description = tooltipPresentationService.buildDescription(
                baseDescriptionResolver.apply(stack),
                config.getSpawnerTooltipMode(),
                tooltipDisplayName,
                roleDisplay,
                gender,
                roleId,
