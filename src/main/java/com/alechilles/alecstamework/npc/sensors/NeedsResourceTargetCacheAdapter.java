...
public final class NeedsResourceTargetCacheAdapter {
    ...
    public Result resolve(@Nonnull Store<EntityStore> store,
                          ...
                          long nowMs) {
        CachedTarget cached = getCachedTarget(npcUuid, worldName, request.resourceKind(), originX, originY, originZ,
                radius, verticalRadius, nowMs);
        if (cached != null) {
            if (cached.target() == null) {
                return Result.miss(cached.reason(), true);
            }
            if (isTargetRejected(npcUuid, request.resourceKind(), cached.target(), nowMs)) {
                cachedTargetsByNpcId.remove(npcUuid, cached);
                clearFastConsumeTarget(npcUuid, worldName, request.resourceKind(), cached.target());
            } else if (isReservedByOther(npcUuid, worldName, request.resourceKind(), cached.target(), nowMs)) {
                cachedTargetsByNpcId.remove(npcUuid, cached);
                clearFastConsumeTarget(npcUuid, worldName, request.resourceKind(), cached.target());
            } else {
                reserveTarget(npcUuid, worldName, request.resourceKind(), cached.target(), nowMs);
                return Result.target(cached.target(), cached.reason(), cached.approachRadius(), cached.fastConsume(), true, null);
            }
        }
        ...
        if (!snapshot.hasCandidates()) {
            if (snapshot.foundConsumableSourceInConsumeRange()) {
                ...
                return Result.target(current, "resource_already_in_consume_range", radius, false,
                        lookup.status() == NeedsResourceSearchCoordinator.Lookup.Status.HIT, null);
            }
            cacheTarget(npcUuid, worldName, request.resourceKind(), null, "resource_target_not_found", radius,
                    verticalRadius, nowMs, false);
        ...
        cacheTarget(npcUuid, worldName, request.resourceKind(), target, reason, candidate.approachRadius(),
                verticalRadius, nowMs, fastModeActive);
        if (fastModeActive) {
        ...
        return Result.target(target, reason, candidate.approachRadius(), fastModeActive, true, candidate);
    }
    ...
    public Result adoptTarget(@Nonnull UUID npcUuid,
        ...
        cacheTarget(npcUuid, worldName, resourceKind, target, "resource_target_search_recent", approachRadius,
                verticalRadius, nowMs, false);
        return Result.target(target, "resource_target_search_recent", approachRadius, false, false, null);
    }
    ...
    public void rememberRecentTarget(@Nullable UUID npcUuid, @Nullable Vector3d target, long nowMs) {
    ...
    public Vector3d resolveRecentTarget(@Nullable UUID npcUuid,
                                        ...
                                        long nowMs) {
        return recentTargetCache.resolve(npcUuid, currentPosition, nowMs);
    }
    /** Forgets a recent destination after a path or consume failure. */
    public void forgetRecentTarget(@Nullable UUID npcUuid, @Nullable Vector3d target) {
    ...
    public void clearTarget(@Nullable UUID npcUuid,
                            ...
                            @Nullable Vector3d target) {
        if (npcUuid != null) {
            CachedTarget cached = cachedTargetsByNpcId.get(npcUuid);
    ...
    @Nullable
    private CachedTarget getCachedTarget(@Nonnull UUID npcUuid,
                                         @Nullable String worldName,
                                         @Nonnull String resourceKind,
                                         double originX,
                                         double originY,
        ...
        if (!isUsable(cached.target(), originX, originY, originZ, radius, verticalRadius)) {
            cachedTargetsByNpcId.remove(npcUuid, cached);
            return null;
        }
        return cached;
    ...
    private void cacheTarget(@Nonnull UUID npcUuid,
                             ...
                             long nowMs,
                             boolean fastConsume) {
        cachedTargetsByNpcId.put(npcUuid, new CachedTarget(
    ...
    public record Result(@Nonnull Status status, @Nullable Vector3d target, @Nonnull String reason, double approachRadius,
                         boolean fastConsume, boolean cacheHit, @Nullable NeedsResourceCandidates.Candidate candidate) {
        private static Result target(@Nonnull Vector3d target, @Nonnull String reason, double approachRadius,
                                     boolean fastConsume, boolean cacheHit, @Nullable NeedsResourceCandidates.Candidate candidate) {
            return new Result(Status.TARGET, target, reason, approachRadius, fastConsume, cacheHit, candidate);
        }
        private static Result miss(@Nonnull String reason, boolean cacheHit) { return new Result(Status.MISS, null, reason, 2.0, false, cacheHit, null); }
        private static Result deferred() { return new Result(Status.DEFERRED, null, "resource_search_deferred", 2.0, false, false, null); }
        private static Result reserved() { return new Result(Status.RESERVED, null, "resource_target_reserved", 2.0, false, true, null); }
    }
