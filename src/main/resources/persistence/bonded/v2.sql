ALTER TABLE bonded_schema_history RENAME TO bonded_v1_schema_history;
ALTER TABLE bonded_companion_profile RENAME TO bonded_v1_profile;
ALTER TABLE bonded_companion_lease RENAME TO bonded_v1_lease;
ALTER TABLE bonded_companion_extension_data RENAME TO bonded_v1_extension_data;
ALTER TABLE bonded_companion_cleanup RENAME TO bonded_v1_cleanup;
ALTER TABLE bonded_companion_operation RENAME TO bonded_v1_operation;
...
CREATE TABLE bonded_companion_profile (
    ...
    snapshot_json TEXT NOT NULL CHECK (
        ...
        AND json_extract(snapshot_json, '$.encoding') IN ('base64', 'hex-utf8')
...
CREATE TABLE bonded_companion_extension_data (
    ...
    json_payload TEXT NOT NULL CHECK (
        json_valid(json_payload)
        AND json_type(json_payload) = 'object'
        AND json_extract(json_payload, '$.encoding') IN ('base64', 'hex-utf8')
        AND json_type(json_payload, '$.payload') = 'text'
        AND length(json_extract(json_payload, '$.payload')) > 0
    ),
    revision INTEGER NOT NULL CHECK (revision >= 0),
    updated_at_ms INTEGER NOT NULL,
...
SELECT profile_id, owner_uuid, roster_id, family_id, role_id, state, revision,
       CASE
           ...
           ELSE json_object(
               'encoding', 'hex-utf8',
               'payload', hex(CAST(snapshot_json AS BLOB))
           )
       END,
...
SELECT profile_id, lease_token, live_npc_uuid, world_key, started_at_ms,
       expires_at_ms, projection_state
FROM bonded_v1_lease;

INSERT INTO bonded_companion_extension_data
SELECT profile_id, namespace,
       json_object(
           'encoding', 'hex-utf8',
           'payload', hex(CAST(json_payload AS BLOB))
       ),
       revision, updated_at_ms
FROM bonded_v1_extension_data;

INSERT INTO bonded_companion_cleanup
SELECT cleanup_id, owner_uuid, roster_id, profile_id, lease_token, target_kind,
       target_npc_uuid, cleanup_reason, cleanup_state, attempt_count,
       next_attempt_at_ms, created_at_ms, retained_until_ms
FROM bonded_v1_cleanup;

INSERT INTO bonded_companion_operation
SELECT caller_namespace, idempotency_key, owner_uuid, roster_id, profile_id,
       operation_type, lower(request_hash), operation_state,
       CASE
           WHEN operation_state = 'PENDING' THEN NULL
           ELSE json_object(
               'code', CASE operation_state
...
FROM bonded_v1_operation;

DROP TABLE bonded_v1_operation;
DROP TABLE bonded_v1_cleanup;
DROP TABLE bonded_v1_extension_data;
DROP TABLE bonded_v1_lease;
DROP TABLE bonded_v1_profile;
DROP TABLE bonded_v1_schema_history;
