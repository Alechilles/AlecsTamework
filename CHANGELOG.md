...
### Fixed
- Fixed an avatar-flight client crash that could become persistent after transformation. Avatar flight now requires Flightmaster's Talisman to be held before model replacement, ignores repeated enable requests for an active session, avoids an extra custom hidden-equipment packet to the local transformed player, and stages the fake rider attachment after the transformed dragon model has reached the client.
