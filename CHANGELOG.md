...
- Fixed permanent-death and restart recovery ordering so a corpse remains retained until the explicit owner release is durably committed, rejected world callbacks cannot strand an in-process hold across unload/reload, and saved revivable corpses restore as non-claim-occupying `DEAD_REVIVABLE` evidence instead of unloaded occupancy.
