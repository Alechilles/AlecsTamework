...
### Fixed
- Fixed linked companions becoming permanently unreachable after their generated instance was deleted. Tamework now preserves their complete state and commits them to Lost recovery during the delete-on-remove world event, before shutdown clears the live snapshot. Relocation diagnostics also distinguish a submitted Lost transition from one that persistence rejected instead of claiming every expired retry was already Lost.
