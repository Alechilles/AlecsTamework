...
- Raised the experimental Public API to `0.9.0` while preserving API 0.8 binary behavior through fail-closed default methods. The contract now declares independent capture-policy, population-group, companion-provisioning, command-family roster, timed-summoning, and paid-revival capabilities; Tamework advertises each one only when its complete runtime authority is installed.
- Added Public API read-only persistence resilience snapshots, mutation-availability queries, and incident lookup for integrations that need the same fail-closed admission result as Tamework.
- Added schema v8 durability for exactly-once capture attempts, population-group classification/count evidence, and idempotent companion-provisioning operations. Schema v9 adds owner/command-family rosters, summon leases and operations, multi-item paid-revival operations, reservations, and exact refund claims. The bounded persistence diagnostics include these domains.
- Added `TwCapturePolicyConfig` and `TwPopulationGroupConfig` asset families, plus opt-in probabilistic capture mechanics on `TwSpawnerConfig`. Existing spawner configs remain deterministic unless they explicitly select `ChanceMode: Probability`.
...
- Upgrading Tamework persistence to schema v8 now creates a verified, transactionally consistent snapshot of Tamework's SQLite database before migration. Tamework never copies the Hytale save or invokes a whole-world backup; Hytale, the host, and the operator remain responsible for complete-save backups.
...
### Fixed
- Fixed historical profiles without persisted role metadata preventing population-group and companion-provisioning capabilities from activating. Those profiles remain explicitly unresolved until an authoritative role is supplied, while unrelated explicit-role operations remain available.
...
- Fixed persistence health diagnostics retaining conservative bootstrap states after a clean startup scan. Durable canonical-profile, owner, world, container, stored/live inventory, claim-occupancy, and loaded-projection coverage now publishes into the named health registry, while incomplete sources remain scoped and fail closed without imposing a login grace period.
...
- Fixed a physically completed same-world Return Home move briefly classifying an unobserved companion as Lost. It now remains Unloaded until the destination projection is observed, while genuinely failed cross-world transfers still become recoverable Lost companions.
...
- Fixed linked companions removed by destructive external cleanup becoming unrecoverable Recall entries. A complete last-live snapshot now transitions unexpected removals to Lost recovery, while ordinary chunk unloads continue to relocate the original NPC. Accepted lost transitions remain authoritative during the removal callback, and recovery reserves its replacement UUID for the original profile before spawning so cleanup recovery cannot create an identity conflict or degrade persistence. A recovered companion can now be cleaned and recovered again even if Hytale rotated its entity UUID after recovery, while finalized historical UUIDs remain suppressed if stale entities reappear. Rejected Recall and lost-transition preflights also leave privacy-bounded telemetry breadcrumbs with their exact reason, and known captured, dead, cooped, or lost states use their specific player warning instead of generic unavailability.
- Fixed finalized lost-recovery sources still serialized in unloaded chunks being counted as duplicate live companions alongside their replacements. Those superseded sources now remain eligible for cleanup without degrading owner-population evidence or blocking same-world and cross-world Recall, and a rejected relocation reports a persistence availability message instead of saying no companions were affected.
