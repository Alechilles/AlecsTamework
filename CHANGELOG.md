...
### Fixed
- Fixed linked companions becoming permanently unreachable after their generated instance was deleted. Tamework now marks them during the delete-on-remove world event, then commits Lost recovery at the entity-removal boundary after live identity is withdrawn but before shutdown clears the complete snapshot. Relocation diagnostics also distinguish a submitted Lost transition from one that persistence rejected instead of claiming every expired retry was already Lost.
