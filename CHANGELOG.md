...
- Needs progression now supports hook-driven resource seeking: thirsty companions can path to adjacent drink positions near water, hungry companions can path to adjacent eat positions near food containers, and a reusable `Component_Tamework_Instruction_Needs_Seek_Resource` behavior handles both flows via parameterized water/food hook states.
