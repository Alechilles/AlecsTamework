...
- Linked panel controls now include explicit `Sort:` and `Filter:` labels, tighter radius control spacing, fixed header-row radius sloting to prevent `Groups` button shifting, and updated panel terminology from `Companions` to `NPCs`.
