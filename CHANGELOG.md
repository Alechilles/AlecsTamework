...
### Changed
- Per-player population limits now count canonical owned companion profiles instead of only loaded NPC entities. Dormant companions retain their ownership world for per-world limits, transfers reserve destination capacity first, and existing over-cap companions remain intact while later acquisitions are blocked. Unavoidable cross-world moves that create an over-cap world now emit a throttled admin warning without deleting the companion.
...
- Fixed incomplete saved-world discovery to keep reconciliation fail-closed, and made claim capability lifecycles release classloader references and refresh after settings changes. Verified provider gates now reject prerelease builds while accepting release build metadata.
