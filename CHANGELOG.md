...
- Raised the experimental Public API to `0.9.0` while preserving API 0.8 binary behavior through fail-closed default methods. The contract now declares independent capture-policy, population-group, companion-provisioning, command-family roster, and timed-summoning capabilities; Tamework advertises each one only when its complete runtime authority is installed.
...
- Added schema v8 durability for exactly-once capture attempts, population-group classification/count evidence, and idempotent companion-provisioning operations. Schema v9 adds owner/command-family rosters, summon leases and operations, and exact capture-source replacement claims for exceptional compensation. The bounded persistence diagnostics include these domains.
