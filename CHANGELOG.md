...
- Added distinct life-stage icons and restored the HUD harvest icon. Command-panel
  ...
  icons use flat white source textures so dynamic tints match their fill colors
  in both the panel and HUD.
...
- Smoothed flock formation turns and position changes, capped formation flight turning at 90 degrees/second, and staggered loose flock heights above and below the leader. Followers also match most of their formation slot's turn movement, reducing catch-up lag while retaining some flexibility.
...
- Prerelease API change: the Husbandry outcomes contract is now `2.0.0`.
  Providers built against the earlier development preview must replace
  `CARE_RESTORATION` and `PRODUCT_BONUS` with the new outcome kinds and use the
  five-field `HusbandryOutcomeModifiers` record. The preview's four-field
  constructor and old multiplier accessors are not part of the released API.
