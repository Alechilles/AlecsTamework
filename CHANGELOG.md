...
- Fixed startup reconciliation binding to a stale loaded-NPC identity snapshot when worlds started after Tamework setup, and fixed a player joining during the longer saved-world scan invalidating already-covered persisted inventory evidence. These failures could leave owner authority unavailable and block taming, ownership changes, and managed-coop lifecycle work.
- Fixed managed-coop duplication paths by transacting canonical profile state, resident slots, and lifecycle operations together; capture sources and stale aliases remain suppressed until retirement is confirmed, and release callbacks require one exact all-world UUID/operation-marker match before adopting or spawning. Daytime release now uses the committed releasing resident instead of the stale housed sweep record, serializes lifecycle pipelines that share the composite index, and waits for startup owner/claim/projection authority instead of degrading persistence while its evidence scan is still running. Restart recovery no longer races a release still owned by the current process. Persisted release adoption rechecks its evidence after asynchronous capacity preparation and cancels only the provisional population reservation if it changed. Managed authority also fails closed unless the observed vanilla coop id matches and its own automatic wild intake is explicitly disabled.
