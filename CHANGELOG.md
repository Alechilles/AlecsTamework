...
- Fixed linked companions removed by destructive external cleanup becoming unrecoverable Recall entries. A complete last-live snapshot now transitions unexpected removals to Lost recovery, while ordinary chunk unloads continue to relocate the original NPC. Accepted lost transitions remain authoritative during the removal callback, and recovery reserves its replacement UUID for the original profile before spawning so cleanup recovery cannot create an identity conflict or degrade persistence.
