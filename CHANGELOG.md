...
### Fixed
- Fixed named linked companions reverting to their species name in the panel after their chunk unloaded or the world restarted. Ordinary unloaded rows now use the latest live state snapshot, then memoized durable profile metadata, before falling back to older command-item metadata.
- Fixed Recall being shown but rejected as unavailable after a released managed-coop companion's chunk unloaded. Deployed residents remain protected by their coop identity while participating in normal Recall relocation, and asynchronously loaded source/destination chunks remain retained until the pending relocation finishes or is canceled. One Recall request now survives the chunk-load/population-reconciliation boundary and internally retries transient admission conflicts; repeated clicks for the same command coalesce instead of replacing the pending request.
...
- Fixed permanent-death and restart recovery ordering so a corpse remains retained until the explicit owner release is durably committed, rejected or accepted-but-never-started world callbacks cannot strand an in-process hold across unload/reload, and saved revivable corpses restore as non-claim-occupying `DEAD_REVIVABLE` evidence instead of unloaded occupancy.
