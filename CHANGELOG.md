...
- Fixed interrupted reconciliation to resume the same persisted scan epoch and source cursors until a `READY` pass, load persisted worlds before catalog construction, and keep failed or incomplete saved-world coverage fail-closed. Claim capability lifecycles now use weak generation-bound contracts, release classloader references, and refresh after settings or plugin setup changes. Verified provider gates reject prerelease builds while accepting release build metadata.
