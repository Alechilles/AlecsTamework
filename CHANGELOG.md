...
- Raised the experimental Public API to `0.9.0` while preserving API 0.8 binary behavior through fail-closed default methods. The contract now declares independent capture-policy, bonded-vessel, population-group, and companion-provisioning capabilities; Tamework advertises each one only when its complete runtime authority is installed.
...
- Added schema v8 durability for exactly-once capture attempts, generation-fenced bonded-vessel bindings and operations, population-group classification/count evidence, and idempotent companion-provisioning operations. The bounded persistence diagnostics and integrity audit include these new domains.
