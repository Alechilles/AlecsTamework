...
<p>Automatic Tamework persistence diagnostics contain a bounded, redacted debug database ZIP and a safe error classification. They exclude the SQLite database, save data, player identity, coordinates, inventory payloads, secrets, exception messages, and unrestricted logs. Set <code>telemetry.enabled</code> to <code>false</code> in Tamework's global settings, or disable <code>Diag</code> in <code>/telemetry consent</code>, to opt out. <code>Diag</code> is separate from Error consent. Existing projects that already reviewed consent must select Save and Close in <code>/telemetry consent</code> before Diagnostics can run.</p>
