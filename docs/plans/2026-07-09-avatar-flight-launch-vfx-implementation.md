...
- Animate scale from roughly `2.5-3.0` down to `0.25-0.50`.
- Opacity should peak below `0.45` and return to zero.
...
- Add modest radial tangent acceleration so dust circles instead of moving in a perfectly straight
  line.
- Keep life below `0.55s`, use strong damping, and cap each pulse at five dust particles.
...
- `ParticleLifeSpan`: approximately `0.20-0.32s`.
...
2. **Charge continues**
   ...
   - Scale is linear interpolation from `0.65` to `1.15` by default.
...
- target particles per pulse: no more than `6`;
- target generated particles: no more than about `40/second`;
- pulse particle life: no more than `0.55s`;
- expected pulse concurrency: below about `24` particles;
