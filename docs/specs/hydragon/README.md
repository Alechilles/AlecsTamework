...
Tamework supplies content-neutral systems required by HyDragon: authoritative capture probability, owner/population admission, canonical profiles, command-family rosters, idempotent provisioning, paid command revival, and public integration capabilities. HyDragon supplies dragons, stones, the Dragon Horn and Wyvern Egg assets, balance, abilities, encounters, and presentation.
...
- Full dragons and Miniwyverns use the same Horn roster with distinct population groups.
- The Wyvern Egg uses provision-and-link and creates no separate Soul Bound Wyvern item.
- HyDragon revival uses configured `Revitalizing_Essence` costs instead of cooldown-only free recovery.
...
| [Command-roster capture and paid revival](command-roster-capture-revival.md) | Spend-on-roll, tame-in-place finalizer, owner-family roster, provision-and-link, paid revival, vessel removal | [Capture and Dragon Horn](https://github.com/Alechilles/HyDragon/blob/main/docs/specs/capture-summoning-maintenance.md) and [Miniwyvern](https://github.com/Alechilles/HyDragon/blob/main/docs/specs/soul-bond-miniwyvern.md) |
...
- Configurable exact inventory costs for command-panel revival.
...
- Full-dragon and Miniwyvern population-group assignment.
- Revitalizing Essence costs by role/difficulty.
...
4. Paid revival and in-front placement.
5. HyDragon conversion and cross-repository tests.
6. Bonded-vessel removal and repository-wide reference cleanup.
7. Clean build, packaged integration, recovery fixtures, diagnostics, and live-server acceptance.
...
6. death preserves the row and paid revival consumes the configured cost once;
7. retries and restart recovery never re-roll, double-spend, duplicate profiles, or duplicate projections;
8. no bonded-vessel or Soul Bound Wyvern path remains;
9. all required diagnostics can run from server console without a player context.
