...
- Death restoration uses Tamework's existing free respawn policy and cooldown fields; HyDragon does not define a payment.
- Recall and respawn prefer safe placement in front of the player.
...
| [Command-roster capture, timed summoning, and death restoration](command-roster-capture-revival.md) | Spend-on-roll, tame-in-place finalizer, owner-family roster, active cap, summon lease/storage, provision-and-link, free respawn integration, vessel removal | [Capture and Dragon Horn](https://github.com/Alechilles/HyDragon/blob/main/docs/specs/capture-summoning-maintenance.md) and [Miniwyvern](https://github.com/Alechilles/HyDragon/blob/main/docs/specs/soul-bond-miniwyvern.md) |
...
- Existing free companion respawn/restoration for dead roster rows.
- In-front-first safe placement for Recall and Respawn.
...
- Role-scoped respawn enablement, cooldowns, and presentation through existing Tamework configuration.
...
- No dragon-specific chance curve, respawn payment, ability, encounter, or presentation logic.
...
12. Respawn restores the existing canonical profile; it never creates a replacement profile or consumes a payment item.
...
5. Free dead-companion respawn integration and in-front placement.
...
6. configured active caps apply to capture, Summon, provisioning, respawn, and recovery;
7. every timed profile expires into roster storage and can be resummoned only after its configured cooldown;
8. death preserves the row and free respawn restores the same profile without consuming an item;
9. retries and restart recovery never re-roll, reset timers, bypass caps, double-spend capture sources, duplicate profiles, or duplicate projections;
