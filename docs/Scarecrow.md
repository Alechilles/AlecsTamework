...
- The preview follows the exact angle at which the scarecrow will be placed.
...
The item inherits the stock `Deco_Scarecrow` block visual and icon. Its preview and placed prop share a unit-scale model wrapper around the same stock model and texture, keeping their size and angle consistent. Those visuals remain an intentionally replaceable placeholder; no custom art is shipped yet.
