...
## Ambient herd journeys

`TameworkAmbientHerd` exposes `READY`, `ACTIVE`, `MOVE`, `DRINK`, and
`REST` phases. Both its `Enabled` and `CanLead` settings default to false.
`READY` offers eligible leaders and admits members only after preparation.
`MOVE` supplies the next shared waypoint; use native `Seek` with
`UsePathfinder: false` and `UseSteering: true` so followers do not each run A*.
`DRINK` supplies a water-facing target while the animal occupies its dry slot.

`TameworkAmbientHerdCancel` cancels the participant's whole herd activity. Use it before switching to taming, threat, sleep, or other
behavior. Clear only the ambient pose before the next behavior installs its
animation. The effective global `AmbientHerds` policy must opt in these roles.
