...
`TameworkOwnerComponent` is the canonical live authorization source. A canonical clear invalidates command-tool links and clears name ownership; a transfer invalidates the prior owner's links and retargets retained name metadata. Damage and command authorization do not treat stale link/name owner IDs as a substitute for canonical ownership.
