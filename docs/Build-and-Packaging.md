...
SQLite is queried read-only for `integrity_check=ok`, WAL, FULL synchronous mode, schema v8, all seven
...
profile/canonical row floor and create a new, non-empty, `tamework_pre_v8_*.sqlite.bak` whose read-only
