...
- While avatar flight is active, the lower-right ability HUD mounts dedicated forward boost, upward flap, airbrake, and crouch-launch artwork in Hytale's native server-content HUD layer so it renders over the built-in weapon glyphs while preserving the native frames and input labels.
