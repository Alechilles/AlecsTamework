...
Generated avatar-flight server models also add missing player-style locomotion aliases that the native transformed-player client can request while grounded: `Sprint` from `Run`, `JumpSprint` from `JumpRun`, and `StepSprint` from `StepRun`. Keep real asset-level keys for these aliases in static avatar models. Runtime animation injection is still useful for forced flight pose clips, but native grounded walk/run/sprint selection can consult the model asset's declared animation set ids before Tamework sends any custom movement animation.
