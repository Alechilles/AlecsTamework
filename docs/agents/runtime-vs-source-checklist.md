...
- Linked-server runtime: `C:\Users\22ale\AppData\Roaming\Hytale\Modding\run\mods\Alechilles_Alec's Tamework!`
- Legacy/manual runtime comparison: `C:\Users\22ale\AppData\Roaming\Hytale\UserData\Mods\alecstamework`
- Current Gradle artifacts: `build/libs`
- Other packaged artifacts: `artifacts`, GitHub release assets, downloaded
  zips, or legacy Maven output under `target`
...
2. Locate the source file and the exact loaded runtime copy. Check the linked
   server runtime before the legacy `UserData\Mods` copy.
3. If a jar/zip is involved, inspect its contents directly. Check `build/libs`
   before legacy `target` artifacts.
...
for archive in build/libs/*.jar target/*.jar; do
  [[ -f "${archive}" ]] && jar tf "${archive}"
done | rg -F "<asset-or-type-id>"
```
...
Use fixed-string searches where possible. Broad regexes are easy to get wrong
against Hytale asset paths and JSON fragments.
