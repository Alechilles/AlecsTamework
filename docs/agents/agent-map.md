...
- The normal development runtime is under `C:\Users\22ale\AppData\Roaming\Hytale\Modding\run\mods`. Treat `UserData\Mods\alecstamework` as a legacy/manual runtime copy. Use the runtime checklist to identify the loaded artifact.
...
- If base-game behavior is unclear, use `hytale-workshop-mcp` and Hytale Workshop MCP. Verify the indexed version and separate evidence from inference. Inspect Tamework source or logs first for mod-specific questions.
...
| Spawners, naming items, icon generation | `docs/Spawner-Config.md`, `docs/Naming-Items.md`, `wiki/Modder-Documentation/Config-Reference/TwDynamicIconConfig-Reference.md` | `scripts/tools/generate_spawner_icon_overrides.py`, `src/main/java/com/alechilles/alecstamework/config/assets/TwDynamicIconConfig.java`, `src/main/java/com/alechilles/alecstamework/config/assets/TwSpawnerConfig.java` |
...
| Runtime systems, tick paths, async work, ECS writes | `$tamework-runtime-safety`, then `docs/agents/guardrails.md` | `src/test/java/com/alechilles/alecstamework/architecture`, runtime checks in guardrails |
| Runtime activation, dormancy, topology reload, activation diagnostics | `$tamework-runtime-safety`, then `docs/Runtime-Activation.md` | `src/main/java/com/alechilles/alecstamework/runtime`, `src/test/java/com/alechilles/alecstamework/runtime` |
...
| Build, release, packaged jars/zips | `docs/Build-and-Packaging.md`, `$alec-mod-release-prep` | `$alec-mod-publish` and the local `mod-release-publisher` for release artifacts and uploads |
| Framework assets or optional examples | `src/main/resources` or `examples/asset-pack`, respectively | `docs/Build-and-Packaging.md`, affected asset consumers and validation |
| New Java regression tests or brittle existing tests | Global Test Value Gate, then `$tamework-test-authoring` when justified | Focused behavior tests and required safety guards; use the verification scope in `AGENTS.md` |
...
- Refresh the generated index when layout changes affect navigation. It is a snapshot, not a completion gate for ordinary code edits. Optional freshness checking is described in `docs/agents/guardrails.md`.
